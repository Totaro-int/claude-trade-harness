...
LLM 트레이딩 하네스의 **첫 실행 온보딩**을 준비하고 서버를 띄운다. 이 플러그인은 런처일 뿐이며, 실제 매매 엔진은 사용자가 클론해 둔 하네스 앱에서 동작한다.
...
4. **이미 설정됨 여부 확인** — `config.json`에 brokerId가 있고 `adapters/`가 차 있으면 이미 온보딩 완료 상태다. 그 경우 `/trade-start`를 안내하고 멈춘다.

5. **온보딩 서버를 백그라운드로 시작** — `npm start`는 미설정 시 자동으로 온보딩 모드로 라우팅된다.
...
- 생성된 어댑터의 `submitOrder`는 검증 대상이 아니다. 라이브 전에 사람이 검토해야 함을 사용자에게 상기시킨다.
